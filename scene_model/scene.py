...
class SceneModel(object):
    ...
    def evaluate_multi(self, **parameters):
        """Evaluate the full model over a range of parameters (eg:
        a cube with varying wavelength).
        """
        # Figure out how long the output will be.
        ...
        for idx in range(length):
            ...
            for name, value in parameters.items():
                if np.isscalar(value):
                    iter_parameters[name] = value
                else:
                    iter_parameters[name] = value[idx]

            iter_model = self.evaluate(**iter_parameters)
            full_model.append(iter_model)
